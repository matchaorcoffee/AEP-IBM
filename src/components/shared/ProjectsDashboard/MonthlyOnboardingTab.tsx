 ...
 *
 * Redesigned "Monthly Onboarding" dashboard tab.
 *
 * Consumes the `monthly-onboarding` chart data from the existing monday.com
 * integration (aggregateByMonth — resources with onboardingStatus "Completed"
 * or "AEP In-Progress" from the last 3 calendar months, bucketed by startDate).
 *
...
import styles from './MonthlyOnboardingTab.module.scss'
...
interface MonthEntry {
  ...
  color: string
  rank: number      // rank by value descending (for table sort)
  chronoIndex: number // original chronological index (0 = oldest)
}

function buildEntries(data: ChartDataPoint[], total: number): MonthEntry[] {
  // Preserve chronological order (backend returns oldest → newest)
  const chrono: MonthEntry[] = data.map((d, i) => ({
    label:        d.label,
    value:        d.value,
  ...
  // Compute rank by descending value (ties broken by chronological order)
  const ranked = [...chrono].sort((a, b) => b.value - a.value || a.chronoIndex - b.chronoIndex)
...
function VerticalBarChart({
  ...
  return (
    <div className={styles.barSection}>
      <h3 className={styles.sectionTitle}>Onboarded resources by month</h3>
...
function DonutChart({
  ...
  return (
    <div className={styles.donutSection}>
      <h3 className={styles.sectionTitle}>Share of onboarded resources</h3>
...
function BreakdownTable({
  ...
  // Sort by value descending for the table (rank display)
  const sorted = [...entries].sort((a, b) => b.value - a.value || a.chronoIndex - b.chronoIndex)

  return (
    <div className={styles.tableSection}>
      <h3 className={styles.sectionTitle}>Monthly onboarding breakdown</h3>
      <table className={styles.table} aria-label="Monthly onboarding breakdown">
        <thead>
          <tr>
...
function EmptyState() {
  return (
    <div className={styles.emptyState}>
      ...
      <p className={styles.emptyTitle}>No resources onboarded in the last 3 months.</p>
      <p className={styles.emptySubtitle}>0 Total onboarded resources</p>
    </div>
  )
}
...
interface MonthlyOnboardingTabProps {
  /** ChartDataPoint[] from aggregateByMonth — 3 items, chronological order */
  data: ChartDataPoint[]
}

export default function MonthlyOnboardingTab({ data }: MonthlyOnboardingTabProps) {
